...
| absolute address 0x00000, in 32-bit protected mode.
	...
	lmsw	ax		| This is it!
	| 此时cpu已经是32保护模式了 jmpi跟的0是段内逻辑偏移地址 8是CS 而此时CS中值的语义是段选择子
	| 0x8=0B 0000 1000的高13位是0x1 也就是说是到GDT中找到1号段描述符 它的段基址是0
...
gdt:
    ...
    | 段2 代码段 代码段的8字节的16进制从高位到低位是00C0 9A00 0000 07FF
    | [55]G=1 段界限的粒度4KB
    ...
    | 段3 数据段 数据段的8字节的16进制从高位到低位是00C0 9200 0000 07FF
    | [55]G=1 段界限的粒度4KB
    | Limit=0x007FF=2047 得到段限长=(2047+1)*4KB=8MB
    | Base=00000000=0x0
    | [44]S=1 是代码段或数据段
